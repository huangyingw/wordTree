...
/**
 * Algorithm:
 *   - to set up the substrings-fullword mapping at reading disk file phase
 *   - to search the full word based on given substring will be O(1). 
 *   - to setup the dictionary and mapping is O(n)+O(
 * 
 * Usage:
 *   - input any keyword 
 *   - output the full words which contains the key words
 *   
 * @author Peng Wang
 *
 */
...
class SpellingCorrector {
	...
	/**
	 * Global single dictionary for mapping each individual substring to it's potential full complete workd which is from disk file
	 */
	private final HashMap<String, List<String>> nwords = new HashMap<String, List<String>>();
	
	private final static String filepath = "c:\\test\\big.txt";
				...
				for(String word: wordlist)
				{
					...
					for(String oneIdx:indexList)
					{
						if(nwords.containsKey(oneIdx))
						{
							...
							//to avoid duplicated words in returned full word list
	...
	/**
	 * 
	 * @param input one single word and return all substring list
	 * @return  e.g input word is the , and return index could be 't' 'h' 'e' , 'th' and 'he'
	 */
	...
	public final String correct(String word) {
		...
		return (ret.toString().length() != 0)
				? ret.toString() : "Word Not Found Contains "+word;
	}
